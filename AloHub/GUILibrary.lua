...
local THEME = {
    ...
    Green = Color3.fromRGB(80, 200, 125),
    Red = Color3.fromRGB(210, 88, 110)
}
...
function Library:CreateWindow(title)
    ...
    corner(main, 20)
    local mainStroke = stroke(main, THEME.StrokeDark, 2, 0.10)
    ...
    UIS.InputChanged:Connect(function(input)
        if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
            ...
            main.Position = UDim2.new(startPos.X.Scale, startPos.X.Offset + delta.X, startPos.Y.Scale,
    ...
    local expandedMain = UDim2.new(0, 760, 0, 470)
    local minimizedMain = UDim2.new(0, 760, 0, 62)
    ...
    local function selectTab(tabObj)
        ...
        tween(tabObj.Button, 0.16, {
            BackgroundColor3 = Color3.fromRGB(28, 23, 44)
        })
        tween(tabObj.Icon, 0.16, {
            TextColor3 = THEME.Accent
        })
        tween(tabObj.Title, 0.16, {
            TextColor3 = THEME.Text
        })
    end
    ...
    function window:CreateTab(name, iconText)
        ...
        function tab:Destroy()
            ...
            if #tabs > 0 and not currentTab then
                selectTab(tabs[1])
            end
        end
        ...
        function tab:AddSlider(text, min, max, default, callback)
            ...
            corner(bar, 99)
            stroke(bar, THEME.StrokeDark, 1, 0.65)
            ...
            corner(knob, 99)
            stroke(knob, THEME.StrokeDark, 1, 0.5)
            ...
            local function updateFromInput(input)
                ...
                local alpha = (input.Position.X - barPos) / math.max(barSize, 1)
            ...
            setValueFromAlpha((default - min) / math.max(max - min, 1))
            ...
            return {
                ...
                Set = function(_, newValue)
                    ...
                    setValueFromAlpha((newValue - min) / math.max(max - min, 1))
                end,
                Holder = holder
            }
        end
        ...
        function tab:AddInput(text, placeholder, callback, hideText)
            ...
            if hideText then
                box:GetPropertyChangedSignal("Text"):Connect(function()
                    ...
                    if #current < #lastMasked then
                        realText = realText:sub(1, #current)
                    else
                        local added = current:sub(#lastMasked + 1)
                        added = added:gsub("•", "")
                        realText = realText .. added
                    end

                    local masked = string.rep("•", #realText)
            ...
            box.FocusLost:Connect(function(enterPressed)
                ...
                local value = hideText and realText or box.Text
                if callback then
            ...
            return {
                Get = function()
                    return hideText and realText or box.Text
                end,
                Set = function(_, value)
                    value = value or ""
                    if hideText then
                        realText = value
                        lastMasked = string.rep("•", #realText)
                        internalUpdate = true
                        box.Text = lastMasked
                        box.CursorPosition = #lastMasked + 1
                        internalUpdate = false
                    else
                        box.Text = value
                    end
        ...
        function tab:AddKeybindRow(text, defaultKey, onChanged)
            ...
            keyBtn.MouseButton1Click:Connect(function()
                ...
                keyBtn.Text = "..."
                tween(keyBtn, 0.15, {
                    BackgroundColor3 = Color3.fromRGB(44, 35, 78)
                })
